...
class ModelEMA(object):
    ...
    def update_skip_dino(self,
                         model: nn.Module,
                        skip_prefix: Tuple[str, ...] = ("dinosaur",) ):
        """
        EMA update, but skip all parameters/buffers whose names start with skip_prefix.
        By default, skip 'dinosaur.*'
        """
        with torch.no_grad():
            self.updates += 1
            d = self.decay_fn(self.updates)
