...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        # cross attention
        target2, sampling_locs, attn_w = self.cross_attn(\
        ...
        return target, sampling_locs, attn_w
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output, sampling_locs, attn_w = layer(output, ref_points_input, memory, memory_spatial_shapes, attn_mask, memory_mask, query_pos_embed)
            all_sampling_locs.append(sampling_locs)
        ...
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), output, all_sampling_locs, all_attn_w
...
class RTDETRTransformerv2(nn.Module):
    ...
    def __init__(self,
        ...
        self._reset_parameters()
        if HSA:
            self.slot_mapper = SlotMapperMLP_tgt(256, 12, 256)
    ...
    def _get_decoder_input(self,
        ...
        # print("self.learn_query_content",self.learn_query_content)
        # print("enc_topk_memory", enc_topk_memory.shape)
        ...
        if slots is not None:
            content = fuse_tgt_slots_with_mlp(content, slots, self.slot_mapper)
    ...
    def forward(self, feats, slots=None, slots1=None, targets=None):
        # print("slots.shape---decoder--v2",slots.shape)
        ...
        init_ref_contents, init_ref_points_unact, enc_topk_bboxes_list, enc_topk_logits_list = \
            self._get_decoder_input(memory, spatial_shapes, slots, denoising_logits, denoising_bbox_unact)

        # decoder
        out_bboxes, out_logits, tgt_query, all_sampling_locs, all_attn_w = self.decoder(
            init_ref_contents,
        ...
        if self.training and self.aux_loss:
            ...
            if dn_meta is not None:
                ...
                out['dn_meta'] = dn_meta
        # if self.training and self.contrastive_loss:
        out['features_tgt'] = tgt_query[:, -self.num_queries:, :] 

        out['sampling_locations'] = all_sampling_locs   # list of tensors
        out['attention_weights']   = all_attn_w         # list of tensors
        out['memory_spatial_shapes'] = spatial_shapes
        return out
...
class SlotMapperMLP_tgt(nn.Module):
    """
    slot (B,1,D) -> (B, segment_length, D)。
    """
    def __init__(self, d_model, segment_length, hidden_dim=512):
        super().__init__()
        self.segment_length = segment_length
        self.net = nn.Sequential(
            nn.Linear(d_model, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, d_model * segment_length)
        )

    def forward(self, slot):
        """
        slot: (B, 1, D)
        return: (B, segment_length, D)
        """
        B, _, D = slot.shape
        slot = slot.squeeze(1)          # (B, D)
        mapped = self.net(slot)         # (B, D * segment_length)
        mapped = mapped.view(B, self.segment_length, D)
        return mapped


def fuse_tgt_slots_with_mlp(tgt, slots, slot_mapper):
    """
    tgt:  (B, total_length, D)
    slots: (B, num_slots, D)
    slot_mapper: (B,1,D) -> (B,segment_length,D)
    """
    segment_length = tgt.size(1) // slots.size(1)
    num_segments = slots.size(1)
    ...
    for i in range(num_segments):
        ...
        tgt_segment = tgt[:, start:end, :]
        
        slot_i = slots[:, i:i+1, :]                # (B,1,D)
        mapped_slot_i = slot_mapper(slot_i)        # (B,segment_length,D)
        
        fused_segment = tgt_segment + mapped_slot_i
        fused_segments.append(fused_segment)
    
    return torch.cat(fused_segments, dim=1)
