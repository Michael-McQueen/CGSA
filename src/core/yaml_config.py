...
class DinosaurConfig:
    def __init__(self,
        ...
        self.encoder = encoder                # encoder type
        self.resize_to = resize_to            # input size (h, w)
        self.patch_size = patch_size          # patch size
        self.enc_channels = enc_channels      # encoder channels
        self.enc_layers = enc_layers          # encoder layers
        
        # Slot parameters
        self.num_slots = num_slots            # number of slots
        self.slot_att_iter = slot_att_iter    # number of iterations
        self.slot_dim = slot_dim              # slot dimension
        
        # training parameters
        self.use_checkpoint = use_checkpoint  # whether to load pretrain
        self.checkpoint_path = checkpoint_path
        
        # auto calculate parameters
        # self.token_num = 400
        self.token_num = (resize_to[0] * resize_to[1]) // (patch_size ** 2)

        self.ISA = ISA  # whether to enable Instance Slot Attention
...
class YAMLConfig(BaseConfig):
    ...
    @property
    def dinosaur(self) -> nn.Module:
        if not hasattr(self, '_dinosaur') or self._dinosaur is None:
            from .hsa_model import DINOSAURpp, Visual_Encoder  # Ensure correct import
            
            dino_cfg = DinosaurConfig(
                ...
                enc_layers=4,
                use_checkpoint=True,
                checkpoint_path=self.yaml_cfg.get("dinosaur_checkpoint", ""),
                ISA=self.yaml_cfg.get("dinosaur_isa", False)
            )
            
            self._dinosaur = DINOSAURpp(dino_cfg).to(self.device)
            self._vis_encoder = Visual_Encoder(dino_cfg).to(self.device)

            # load pretrain weights
            if dino_cfg.use_checkpoint and dino_cfg.checkpoint_path:
                checkpoint = torch.load(dino_cfg.checkpoint_path, map_location=self.device, weights_only=False)
                state_dict = {k.replace("module.", ""): v for k, v in checkpoint["model"].items()}
                self._dinosaur.load_state_dict(state_dict, strict=False)

        return self._dinosaur

    @property
    def vis_encoder(self) -> nn.Module:
        if not hasattr(self, '_vis_encoder') or self._vis_encoder is None:
            _ = self.dinosaur
        return self._vis_encoder
    ...
    def optimizer(self, ) -> optim.Optimizer:
        if self._optimizer is None and 'optimizer' in self.yaml_cfg:
            ...
            # ensure dinosaur parameters are parsed correctly
            dinosaur_params = list(self.dinosaur.parameters()) + list(self.vis_encoder.parameters())

            # force convert dinosaur_lr to float
            dinosaur_lr = float(self.yaml_cfg.get('dinosaur_lr', 1e-5))

            # add dinosaur parameters
            params.append({'params': dinosaur_params, 'lr': dinosaur_lr})

            # debug info
            # print(f"DEBUG: optimizer params = {params}")
