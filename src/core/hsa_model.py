...
class DINOSAURpp(nn.Module):
    def __init__(self, args):
        ...
        # parameters
        self.slot_dim = args.slot_dim
        self.num_slots = args.num_slots  # first layer slot number (5)
        # self.num_slots = args.sub_slots  # second layer slot number (5)
        self.token_num = args.token_num
        
        # encoder
        self.encoder_l1 = ISA(args, input_dim=768) if args.ISA else SA(args, input_dim=768)
        self.encoder_l2 = ISA(args, input_dim=768) if args.ISA else SA(args, input_dim=768)
        
        # decoder
        self.decoder_l1 = Decoder(args)  # first layer reconstruction
        self.decoder_l2 = Decoder(args)  # second layer reconstruction
        # self.decoder_sub = Decoder(args)  # second layer sub-graph reconstruction
        self.pos_dec = nn.Parameter(torch.Tensor(1, self.token_num, self.slot_dim))
        init.normal_(self.pos_dec, mean=0., std=.02)

    # def hierarchical_reconstruct(self, slots_l1, slots_l2):
    #     """Hierarchical reconstruction logic"""
    #     # First layer reconstruction
    #     slot_maps_l1 = self.sbd_slots(slots_l1)
    #     recon_l1, _ = self.reconstruct_feature_map(self.decoder_l1(slot_maps_l1))
        
    #     # Second layer reconstruction (each L1 slot corresponds to 5 L2 slots)
    #     losses_l2 = []
    #     for i in range(self.num_slots):
    #         # Extract 5 L2 slots corresponding to current L1 slot
    #         slots_sub = slots_l2[:, i*self.num_slots : (i+1)*self.num_slots, :].contiguous()
    #         # print("slots_sub", slots_sub.shape)
            
    #         # Generate parent slot features
    #         parent_feat = slots_l1[:, i:i+1, :].expand(-1, self.num_slots, -1)
            
    #         # # Calculate sub-slot reconstruction loss
    #         # slot_maps_sub = self.sbd_slots(slots_sub)
    #         # slot_maps_sub = self.decoder_l1(slot_maps_sub)
    #         # recon_sub, _ = self.reconstruct_feature_map(slot_maps_sub)
    #         # print("slot_maps_sub", slot_maps_sub.shape)
    #         # print("parent_feat", parent_feat.shape)
    #         loss_sub = F.mse_loss(slots_sub, parent_feat.detach())
    #         losses_l2.append(loss_sub)
            
    #     return recon_l1, sum(losses_l2)/len(losses_l2)
    ...
    def compute_weighted_slots(self, masks_l2, feature_tokens):
        """
        Args:
            masks_l2: (B, S2, token)  # masks returned from reconstruct_feature_map
            feature_tokens: (B, token, 768)  # patch features output from image encoder
        Returns:
            weighted_slots: (B, S2, 768)  # each slot corresponds to a weighted visual feature
        """
        B, S2, T = masks_l2.shape
        _, T_f, D = feature_tokens.shape
        assert T == T_f, "Token count mismatch!"

        # Normalize masks_l2 to ensure soft aggregation
        masks_norm = masks_l2 / (masks_l2.sum(dim=-1, keepdim=True) + 1e-8)  # (B, S2, token)

        # Weighted sum
        weighted_slots = torch.einsum('bsk,btd->bsd', masks_norm, feature_tokens)
        # bsk: batch, slot, token
        # btd: batch, token, dim
        # -> bsd: batch, slot, dim
    ...
    def forward(self, features):
        ...
        slots_l1 = self.encoder_l1(features)  # (B, S, D_slot), (B, S, token)
        assert torch.sum(torch.isnan(slots_l1)) == 0

        # First layer reconstruction
        slot_maps_l1 = self.sbd_slots(slots_l1)
        slot_maps_l1 = self.decoder_l1(slot_maps_l1)                            # (B, S, token, 768 + 1)

        recon_l1, masks_l1 = self.reconstruct_feature_map(slot_maps_l1)     # (B, token, 768), (B, S, token)

        # Split the last dimension, extract the first 768 features
        slot_maps_features = slot_maps_l1[:, :, :, :768]  # (B, S, token, 768)

        # Split slot_maps_features along S dimension into five tensors
        slot_maps_list = torch.split(slot_maps_features, 1, dim=1)  # each tensor shape: (B, 1, token, 768)

        # Remove redundant dimensions
        slot_maps_list = [s.squeeze(1) for s in slot_maps_list]  # each tensor shape: (B, token, 768)
        
        # Second layer encoding (each L1 slot generates 5 L2 slots)
        slots_l2 = []
        losses_l2 = []
        for i in range(self.num_slots):
            sub_slots = self.encoder_l2(slot_maps_list[i])  # (B, token, 768)

            # sub_slot_maps = self.sbd_slots(sub_slots)
            # sub_slot_maps = self.decoder_sub(sub_slot_maps)                            # (B, S, token, 768 + 1)

            # recon_sub, masks_sub = self.reconstruct_feature_map(sub_slot_maps)     # (B, token, 768), (B, S, token)

            # loss_sub = F.mse_loss(recon_sub, slot_maps_list[i].detach())
            # losses_l2.append(loss_sub)

            slots_l2.append(sub_slots)

        slots_l2 = torch.cat(slots_l2, dim=1)  # (B,S*S,D)

        assert torch.sum(torch.isnan(slots_l2)) == 0

        # loss_l2_sub = sum(losses_l2)/len(losses_l2)

        loss_l2_sub = 0

        # Second layer reconstruction
        slot_maps_l2 = self.sbd_slots(slots_l2)
        slot_maps_l2 = self.decoder_l2(slot_maps_l2)                            # (B, S, token, 768 + 1)

        recon_l2, masks_l2 = self.reconstruct_feature_map(slot_maps_l2)     # (B, token, 768), (B, S, token)
        weighted_slots = self.compute_weighted_slots(masks_l2, features)  # (B, 25, 768)
        # print("weighted_slots", weighted_slots.shape)
        return recon_l2, slots_l2, masks_l2, loss_l2_sub, recon_l1, masks_l1, slots_l1, weighted_slots